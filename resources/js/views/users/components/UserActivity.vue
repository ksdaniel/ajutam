<template>
  <el-card v-if="user.name">
    <el-tabs v-model="activeActivity" @tab-click="handleClick">
      ...
      <!--</div>-->
      <!--</el-tab-pane>-->
      <el-tab-pane v-loading="updating" :label="$t('account.accountUp')" name="second">
        <el-form-item label="Name">
          <el-input v-model="user.name" :disabled="user.role === 'admin'" />
        </el-form-item>
        <el-form-item v-if="checkRole(['admin'])" label="Email">
      ...
      <el-tab-pane v-loading="updating" :label="$t('account.changePassword')" name="third">
        <el-form-item label="New password">
          <el-input ref="password" v-model="user.password" v-validate="'required'" data-vv-scope="third" type="password" name="password" :disabled="user.role === 'admin'" />

          <span v-if="errors.first('third.password')" style="color:red">{{ $t('changePassword.password') }}</span>
        </el-form-item>
        <el-form-item label="Confirm password">
          <el-input v-model="confirmPassword" v-validate="'required|confirmed:password'" data-vv-scope="third" type="password" name="confirm_password" :disabled="user.role === 'admin'" />

          <span v-if="errors.first('third.confirm_password')" style="color:red">{{ $t('changePassword.changePassword') }}</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :disabled="user.role === 'admin'" @click="onSubmit">
            {{ $t('account.update') }}
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$validator.validate(this.activeActivity + '.*').then((result) => {
        if (result) {
          this.updating = true;
